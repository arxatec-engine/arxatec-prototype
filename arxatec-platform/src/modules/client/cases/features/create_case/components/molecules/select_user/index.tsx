...
interface Props {
  ...
  onSelect: (user: LawyerModel) => void;
  lawyers: LawyerModel[];
}
...
export const SelectUser: React.FC<Props> = ({
  ...
  const [selectedPerson, setSelectedPerson] = useState<LawyerModel | null>(
    null
  );

  const filteredPeople: LawyerModel[] =
    query === ""
      ? []
  ...
  const handleSelect = (lawyer: LawyerModel) => {
    onSelect(lawyer);
    handleClose();
  };
                      ...
                      <div className="hidden h-96 w-1/2 flex-none flex-col divide-y divide-gray-100 overflow-y-auto sm:flex">
                        ...
                        <div className="flex flex-auto flex-col justify-between p-6">
                          <dl className="grid grid-cols-1 gap-x-6 gap-y-3 text-sm text-gray-700">
                            ...
                            <dd>{activeOption.licenseNumber}</dd>
                            <dt className="col-end-1 font-semibold text-gray-900">
                              Email
                            </dt>
